...
export class Monster extends ex.Actor {
  ...
  async queryForTarget() {
    // Если у нас нет действительной цели
    if (!this.target || this.target?.isKilled()) {
      // Запросите всех игроков на карте
      const playersQuery = this.scene.world.queryManager.createQuery([TAG_ANY_PLAYER]);
  
      // Фильтрация до ближайших, в пределах пиксельного диапазона
      ...
      // Если у нас есть результаты, выберите случайного, чтобы нацелиться
